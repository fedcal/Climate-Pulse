...
**Domain:** Multi-source weather/time-series data aggregation pipeline (ARPA scraping + ECMWF Open Data + NOAA METAR + Copernicus C3S → TimescaleDB → FastAPI public API + Angular dashboard)
**Researched:** 2026-05-23
**Confidence:** HIGH for core stack and well-known auxiliaries; MEDIUM for some Angular wrapper version pins (ecosystem moves fast)

## Executive Note

The owner has pre-decided the architectural backbone (FastAPI 0.115+, Python 3.12, TimescaleDB, Angular 21 SSR, Leaflet, Plotly, Celery, Redis). All recommendations below validate and refine those choices with current (2026) versions and prescribe the specific glue libraries needed to assemble the system. Constraints respected: self-hosted Docker Compose, zero cloud budget, solo developer.
...
```bash
# --- Python (uv) ---
uv init climate-pulse
uv add fastapi==0.136.* uvicorn[standard] gunicorn pydantic pydantic-settings orjson
uv add sqlalchemy[asyncio]==2.0.* asyncpg sqlalchemy-timescaledb alembic
uv add celery[redis]==5.6.3 redis flower
uv add httpx selectolax parsel lxml tenacity aiocache pyarrow
uv add ecmwf-opendata cdsapi cfgrib xarray pandas numpy metpy pint
uv add slowapi structlog
uv add opentelemetry-distro[otlp] opentelemetry-instrumentation-fastapi \
...
uv add --dev pytest pytest-asyncio pytest-cov hypothesis \
              testcontainers[postgres] respx vcrpy \
...
# --- Angular (npm) ---
ng new climate-pulse-dashboard --ssr --style=scss --routing
cd climate-pulse-dashboard
...
npm install --save-dev @types/leaflet @playwright/test
```
...
**If ingest source returns GRIB (ECMWF, CDS):**
...
**If ingest source returns HTML (some ARPA regional sites):**
...
**If ingest source returns JSON/CSV (ARPA REST endpoints, METAR TXT):**
...
**If a query endpoint returns >10k rows:**
...
**If a chart needs >10k points (Plotly):**
...
---
*Stack research for: Climate Pulse — multi-source weather aggregation pipeline*
*Researched: 2026-05-23*
